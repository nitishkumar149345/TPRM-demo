...
from agents.report_agent import ReportWriter
...
async def process_document(
    ...
    return JSONResponse(
        content={
            "message": f"document uploaded to vector db in collection:{vendor_id}"
        },
...
async def extract_target_metrics(
    ...
    extractor = ExtractionEngine(
        document_id=document_id, collection_id=utils.serialize_uuid(vendor_id)
    )
...
@app.post("/report")
async def generate_report(comparison_data: dict):
    try:
        agent = ReportWriter(comparison_json=comparison_data)
        response = agent()
        report = response.content
        return Response(content=report, status_code=status.HTTP_200_OK)
    except Exception as e:
        raise Exception(e)
